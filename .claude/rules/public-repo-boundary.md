...
`EWS-Consulting-Public/ews-ci-action` is **world-readable**. The other EWS
...
This has already happened once: an issue was opened here using a body template
written for private repositories, which named internal repositories and local
...
repository deliberately carries **no long-running internal notes issue**, and
why one must not be created.
