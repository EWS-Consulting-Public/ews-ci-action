...
- **Never trust pre-2026-08-07 documentation without verifying against the
  YAML.** Eleven old claims were wrong; corrections are in `docs/README.md`
  and the ADRs.
...
## How a credential reaches the runner

Every key of `EWS_CREDENTIALS` is exported to the job as `UPPER(key)` — the
flat environment name the EWS configuration layer reads — after validation
against `EWS_CREDENTIALS_KEYS`. **A new credential needs no change here**:
add it where the payload is composed, re-run the tooling on the repository,
and the runner has it. Never add a `jq` extraction for a named key; never
write a new TOML file for one. `EWS_DATA_ROOT` is exported the same way, and
`ci.yml`'s `data-plan:` input caches that directory on a plan's committed
lock (ADR 0006).
